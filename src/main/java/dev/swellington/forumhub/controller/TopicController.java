...
public class TopicController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TopicDetailDto> update(@PathVariable Long id, @RequestBody @Valid TopicUpdateDto dto) {
        return ResponseEntity.ok(TopicDetailDto.fromTopic(topicService.update(id, dto)));
    }

    @GetMapping("/{id}/responses")
    public ResponseEntity<Page<ResponseDetailDto>> getAllTopicResponse(@PathVariable Long id, @PageableDefault(sort = {"createdAt"}) Pageable pageable) {
